(function() {
    ...
    function ajax(method, url, data, callback, errorHandler) {
        ...
        xhr.open(method, url, true);            //将数据传给后端  后端传回来

        xhr.onload = function() {               //得到的结果 判断 正不正确
            if (xhr.status === 200) {
    ...
    function loadNearbyItems() {
        ...
        var url = './search';
        if (check == 1) {
            lat = document.getElementById("form-control-lan").value;
            lng = document.getElementById("form-control-lon").value;
        }
        console.log(lat)
        console.log(lng)
    ...
    var zhengchang = document.querySelector("#zhengchang");
    resetButton.addEventListener("click",function(){
        check = 1;
        init();
    })
    zhengchang.addEventListener("click", function () {
        check = 0;
        init();
    })
    check = 0
